...
export function ErrorState({
  ...
  return (
    ...
    >
      ...
      <h3 className="text-[14px] font-semibold leading-5 text-neutral-900">{t('common.couldNotLoad')}</h3>
      <p className="mt-1 max-w-[380px] text-caption text-neutral-500">
        {message ?? t('common.couldNotLoadBody')}
      </p>
      {onRetry && (
...
export function TableSkeleton({ rows = 6, columns = 6 }: { rows?: number; columns?: number }) {
  ...
  return (
    <div aria-busy="true" aria-live="polite">
      <span className="sr-only">{t('common.loadingRecords')}</span>
